  ...
  methods: {
    ...
    deleteUser(id) {
      this.globalService.showConfirmMessage(
        " you want to delete this user?",).then(
        () => {
          UsersService.deleteUserById(id).then((response) => {
            this.users = this.users.filter((user) => {
              return user.id != id;
            });
            this.globalService.toasterShowSuccess("User deleted successfully");
          });
        }
      )
...
<template>
  ...
  <div class="content">
    ...
    <BaseBlock :title="`  User in Total`" content-full class="animated zoomIn">
      <template #options>
        <div class="space-x-1">
          <button
            type="button"
