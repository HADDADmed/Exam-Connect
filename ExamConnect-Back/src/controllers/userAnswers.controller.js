     ...
     async (req, res, next) => {
          ...
          const selectQuery = `
            SELECT
                ...
                exam_user_status.status_id AS exam_user_status_id,
                exam_user_status.createdAt AS exam_user_status_createdAt,
                exam_status.status AS exam_status_name
            FROM userAnswer
            ...
            LEFT JOIN exam_user ON userAnswer.exam_id = exam_user.exam_id
               LEFT JOIN exam_user_status ON exam_user.id = exam_user_status.exam_user_id
               LEFT JOIN exam_status ON exam_user_status.status_id = exam_status.id
               WHERE 
                    userAnswer.user_id = ?
               AND
               ...
               (err, rows) => {
                    ...
                    rows.forEach((row) => {
                         ...
                         if (!existingQuestionOption) {
                           ...
                           // historyOfStatus
                         const existingStatus = userAnswer.historyOfStatus.find(
                              (status) =>
                                   status.exam_user_status_id ===
                                   row.exam_user_status_id
                         );
                         if (!existingStatus) {
                              const status = {
                                   exam_user_status_id:
                                        row.exam_user_status_id,
                                   exam_user_status_createdAt:
                                        row.exam_user_status_createdAt,
                                   exam_status_name: row.exam_status_name,
                              };
                              userAnswer.historyOfStatus.push(status);
                         }
                    });
                    ...
                    userAnswer.questions.forEach((question) => {
                         ...
                         // calculate correctQuestions and wrongQuestions isQcm === 1 || isQcm === 2
                         ...
                         // calculate correctTextQuestions and wrongQuestions isQcm === 0
                         if (question.isQcm == 0 ){
                              if (question.isTrue === 1) {
                    ...
                    // sort historyOfStatus by date
                    userAnswer.historyOfStatus.sort((a, b) => {
                         return (
                              new Date(b.exam_user_status_createdAt) -
                              new Date(a.exam_user_status_createdAt)
                         );
                    });
     ...
     async (req, res, next) => {
          ...
          const selectQuery = `
            SELECT
                ...
                exam_status.status AS exam_status_name
               FROM userAnswer
               INNER JOIN Exam ON userAnswer.exam_id = Exam.id
               INNER JOIN user ON userAnswer.user_id = user.id
               INNER JOIN question ON userAnswer.question_id = question.id
               INNER JOIN questionOption ON userAnswer.questionOption_id = questionOption.id
               INNER JOIN exam_user ON userAnswer.exam_id = exam_user.exam_id AND  userAnswer.user_id = exam_user.user_id
               INNER JOIN exam_user_status ON exam_user.id = exam_user_status.exam_user_id
               INNER JOIN exam_status ON exam_user_status.status_id = exam_status.id
               WHERE 
                    userAnswer.user_id = ?
               AND
                    userAnswer.exam_id = ?
