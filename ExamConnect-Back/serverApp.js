...
const storage = multer.diskStorage({
  ...
  app.post('/questions', (req, res) => {
    const { text_question, type_question, choices, correctChoiceIndex } = req.body;
    const query = 'INSERT INTO questions (texte_question, type_question) VALUES (?, ?)';
    connection.query(query, [text_question, type_question], (err, result) => {
        if (err) {
            ...
            if (choices && choices.length > 0) {
                const choiceQuery = 'INSERT INTO choix (text_question, question_id, status) VALUES (?, ?, ?)';
                choices.forEach((choice, index) => {
                    let choiceText = choice.type === 'text' ? choice.value : choice.source; 
                    let choiceStatus = index === correctChoiceIndex ? 'active' : 'inactive';
                    connection.query(choiceQuery, [choiceText, question_id, choiceStatus], (err, choiceResult) => {
                        if (err) {
            ...
            res.status(201).json({ message: 'Question et choix enregistrés avec succès' });
        }
    });
  });
  ...
  connection.query(query, (err, results) => {
    if (err) {
      ...
      const exams = [];
      results.forEach(row => {
        const existingExam = exams.find(exam => exam.examen_id === row.examen_id);
        ...
        if (!existingExam) {
          exams.push({
            ...
            questions: [{ id: row.question_id, texte_question: row.texte_question, type_question: row.type_question }]
          });
        } else {
          existingExam.questions.push({ id: row.question_id, texte_question: row.texte_question, type_question: row.type_question });
        }
      });
  ...
  app.post('/exams', (req, res) => {
    const { titre, nombre_qsts, date_heure_debut, date_heure_fin, questions } = req.body;

    const examQuery = 'INSERT INTO examens (titre, nombre_qsts, date_heure_debut, date_heure_fin) VALUES (?, ?, ?, ?)';

    connection.query(examQuery, [titre, nombre_qsts, date_heure_debut, date_heure_fin], (err, examResult) => {
        if (err) {
            console.error('Erreur lors de l\'enregistrement de l\'examen:', err);
            res.status(500).json({ message: 'Erreur lors de l\'enregistrement de l\'examen' });
        } else {
            const examen_id = examResult.insertId; 
            
            if (questions && questions.length > 0) {
                const examQuestionQuery = 'INSERT INTO examen_question (examen_id, question_id) VALUES (?, ?)';
                questions.forEach(question => {
                    const question_id = question.id; 
                    connection.query(examQuestionQuery, [examen_id, question_id], (err, examQuestionResult) => {
                        if (err) {
                            console.error('Erreur lors de l\'enregistrement de la question de l\'examen:', err);
                        }
                    });
                });
            }    
  ...
  connection.query(query, [examenId], (err, results) => {
    if (err) {
      console.error("Erreur lors de la récupération de l'examen et des questions:", err);
      res.status(500).json({ message: "Erreur lors de la récupération de l'examen et des questions" });
    } else {
...
app.get("/exams/:examenId/questions", (req, res) => {
  ...
  connection.query(query, [examenId], (err, results) => {
    if (err) {
      ...
      res.status(500).json({ message: "Erreur lors de la récupération des questions de l'examen" });
    } else {
  ...
  app.post('/register', (req, res) => {
    ...
    const query = 'INSERT INTO utilisateur (nom, prenom, email, mot_de_passe, level) VALUES (?, ?, ?, ?, ?)';
  
    connection.query(query, [nom, prenom, email, mot_de_passe, level], (err, result) => {
      if (err) {
...
app.listen(process.env.PORT || PORT, () => {
    console.log(`Serveur en cours d'exécution sur http://localhost:${process.env.PORT || PORT}`);
});
